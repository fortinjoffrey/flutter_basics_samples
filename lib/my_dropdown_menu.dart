...
class _MyDropdownMenuState<T> extends State<MyDropdownMenu<T>> {
  ...
  void initState() {
    ...
    final int index =
        filteredEntries.indexWhere((MyDropdownMenuEntry<T> entry) => entry.value == widget.initialSelection);
    if (index != -1) {
  ...
  int? search(List<MyDropdownMenuEntry<T>> entries, TextEditingController textEditingController) {
    ...
    final int index =
        entries.indexWhere((MyDropdownMenuEntry<T> entry) => entry.label.toLowerCase().contains(searchText));

    return index != -1 ? index : null;
  }

  List<Widget> _buildButtons(List<MyDropdownMenuEntry<T>> filteredEntries, TextEditingController textEditingController,
    ...
    for (int i = 0; i < filteredEntries.length; i++) {
      ...
      final Color focusedBackgroundColor =
          effectiveStyle.foregroundColor?.resolve(<MaterialState>{MaterialState.focused}) ??
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> menu =
        _buildButtons(filteredEntries, _textEditingController, textDirection, focusedIndex: currentHighlight);
    ...
    if (widget.width != null) {
      effectiveMenuStyle =
          effectiveMenuStyle.copyWith(minimumSize: MaterialStatePropertyAll<Size?>(Size(widget.width!, 0.0)));
    } else if (anchorWidth != null) {
      effectiveMenuStyle =
          effectiveMenuStyle.copyWith(minimumSize: MaterialStatePropertyAll<Size?>(Size(anchorWidth, 0.0)));
    }
        ...
        child: MenuAnchor(
          ...
          builder: (BuildContext context, MenuController controller, Widget? child) {
            ...
            final Widget leadingButton =
                Padding(padding: const EdgeInsets.all(8.0), child: widget.leadingIcon ?? const SizedBox());
              ...
              children: <Widget>[
                TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      prefixIcon:
                          widget.leadingIcon != null ? Container(key: _leadingKey, child: widget.leadingIcon) : null,
